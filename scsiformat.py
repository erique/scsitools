...
def calculate_bpb(partition_records, root_entries=1024):
    ...
    # Need SPC>1: target max 32768 clusters (= 64 FAT records)
    for spc in [2, 4, 8, 16, 32, 64, 128]:
        fat_recs, clusters = solve_fat_recs(partition_records, spc)
        if clusters <= 32768 and fat_recs <= 255:
...
def make_dir_entry(name, ext, attr, start_cluster, file_size, time=0, date=0):
    ...
    # +0x00 (8B): filename (padded with spaces)
    entry[0x00:0x08] = name.encode("ascii").ljust(8, b" ")
    # +0x08 (3B): extension (padded with spaces)
    entry[0x08:0x0B] = ext.encode("ascii").ljust(3, b" ")
    # +0x0B (1B): attributes
    entry[0x0B] = attr
    # +0x0C-0x15: reserved (zeros)
...
def write_root_directory(f, partition_offset, fat_recs, root_dir_recs, file_entries, verbose=False):
    """Write root directory entries.

    file_entries is a list of (name, ext, attr, start_cluster, file_size, time, date) tuples.
    """
    ...
    for i, (name, ext, attr, start_cluster, file_size, time, date) in enumerate(file_entries):
        entry = make_dir_entry(name, ext, attr, start_cluster, file_size, time, date)
        offset = i * DIR_ENTRY_SIZE
        root[offset:offset + DIR_ENTRY_SIZE] = entry
        if verbose:
            print(f"    {name}.{ext}: cluster={start_cluster}, size={file_size}, attr=0x{attr:02X}")
...
def main():
    ...
    with open(args.image_file, "r+b") as f:
        ...
        # HUMAN.SYS: attr 0x24 (system + archive), COMMAND.X: attr 0x20 (archive)
        # Timestamps from reference images
        file_entries = [
